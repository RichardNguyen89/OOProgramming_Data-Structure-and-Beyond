...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
	...
	public boolean addWord(String word)
	{
		...
		if (!current.endsWord()) {
			...
			// check whether the character is in the node
			// if it is in, set cur = that node (or the node child??? think about it)
			
			// if it is not, create a new node
			
			// reach the end, turn the word flag into true;
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 if (current.endsWord()) { completions.add(current.getText());}
    	 
    	 Queue<TrieNode> queue = new LinkedList<TrieNode>();
    	 
    	 List<Character> children = new LinkedList<Character>(current.getValidNextCharacters());
